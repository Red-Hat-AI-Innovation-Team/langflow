...
class RunFlowBaseComponent(Component):
    ...
    def _get_data_fields_from_vertex(
        ...
        for field_name in field_names:
            ...
            field_config = dict(field_template[field_name])
            if not isinstance(field_config, dict):
                continue

            # Map "other" type to Any for Pydantic schema compatibility
            # "other" type fields are complex objects (DataFrame, Data, etc.)
            if field_config.get("type") == "other":
                field_config["type"] = Any
